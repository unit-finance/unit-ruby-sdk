...
RSpec.describe Unit::Webhook do
  ...
  it "updates a webhook" do
    response = described_class.update_webhook(webhook_id: "3657", label: "updated webhook", url: "https://webhook.site/81ee6b53-fde4-4b7d-85a0-0b6249a4488d",
                                              content_type: "Json", token: "myT")
    expect(response.data["type"]).to eq("webhook")
  end

  it "gets a webhook" do
    response = described_class.get_webhook(webhook_id: "3657")
    expect(response.data["type"]).to eq("webhook")
  end

  it "enables a webhook" do
    response = described_class.enable_webhook(webhook_id: "3657")
    expect(response.data["type"]).to eq("webhook")
  end

  it "disables a webhook" do
    response = described_class.disable_webhook(webhook_id: "3657")
    expect(response.data["type"]).to eq("webhook")
  end
end
